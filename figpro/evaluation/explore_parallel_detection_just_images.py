...
for k in res3.keys():
    if k in ids_match and k in res1.keys() and len(res1[k]) > 1:
        ...
        for i in range(len(res1[k])):
            new_str = ""
            new_str += " ".join(res1[k][:i])
            if i == 0:
                new_str += "[" + res1[k][i] + "]"
            else:
                new_str += " [" + res1[k][i] + "]"
            if i < len(res1[k])-1:
                new_str += " " + " ".join(res1[k][i+1:])
        ...
        avg_score = sum_score / len(res1[k])
        print("Average Compatibility Score: " + str(avg_score))
        print("Truth: " + res4[k])
        sum_p[res4[k]] += avg_score
        count_p[res4[k]] += 1
        blabla.append((avg_score, res4[k], k))
...
import csv 
with open('explore_parallel_detection_just_images.csv', 'w') as csv_file:
    ...
    for b in blabla:
        ...
        if (b[1] == 'parallel' and b[0] >= cur_thres) or (b[1] == 'non_parallel' and b[0] < cur_thres):
            total_c += 1
            writer.writerow([b[2], 'correct', b[1], str(b[0]), "\t".join(img_obj_labels)])
        else:
            total_w += 1
