...
def _compute_sampled_logits(weights,
  ...
  with ops.name_scope(name, "compute_sampled_logits",
    ...
    sampled = sampled[:NEGATIVE_SAMPLING_NUM*batch_size]
    ...
    inputs_re = array_ops.reshape(inputs,[batch_size,-1,dim])
    sampled_w_re = array_ops.reshape(sampled_w,[batch_size,-1,dim])

    sampled_logits = math_ops.multiply(inputs_re,sampled_w_re)
    sampled_logits = array_ops.reshape(sampled_logits,[-1,dim])
    sampled_logits = _sum_rows(sampled_logits)
    sampled_logits = array_ops.reshape(sampled_logits,[batch_size,-1])
